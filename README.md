...
- Live Preview (`.livePreviewEnabled(true)` / `isLivePreviewEnabled`): syntax markers (heading `#`, emphasis / strikethrough / inline-code delimiters, link and image brackets, blockquote `>`) are drawn with (practically) zero width on every line except the ones the caret or selection touches; while the editor is not first responder (keyboard dismissed on iOS, another view focused on macOS) every line hides them, so an unfocused note reads like a rendered page. The text storage is never changed — copying still yields plain Markdown. Code fences and task checkboxes stay visible.
- Live Preview tables: a GFM table that starts at a line start is drawn as a grid (column alignment from the delimiter row, inline styles kept, cells wrap when the table is wider than the text) while the caret is outside it or the editor is not first responder; a click / tap on a cell puts the caret at the end of that cell and shows the Markdown source of the whole table. The same mechanism renders Front Matter as a key / value table.
...
- Block decorations: code blocks as one full-width rounded box with vertical padding (`MarkdownTheme.codeBlockVerticalPadding`; a code block that starts at the very first paragraph gets no top padding because TextKit ignores `paragraphSpacingBefore` there), blockquote bars drawn in a display-only paragraph indent (`MarkdownTheme.blockquoteIndent`, default 16 pt) so the bar keeps one x across every line and stays clear of the text, thematic-break rules, table backgrounds
