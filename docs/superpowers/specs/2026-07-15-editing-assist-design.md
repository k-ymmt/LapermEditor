...
- **空項目脱出**: 行がマーカーだけ(本文が空)なら、インデントを含む行内容全体を削除して空行にする
...
- オフセットがタスク項目行の `[ ]` / `[x]` の 3 文字範囲内にあるときだけ、中央の 1 文字を置換(` ` → `x`、`x` / `X` → ` `)し、イベントを消費する
- カーソル(選択範囲)は動かさない: 置換は同長 1 文字で座標がずれないため、UI 層は適用前の選択範囲を適用後に復元する(この経路では `EditCommand.selectedRange` を使わない)
